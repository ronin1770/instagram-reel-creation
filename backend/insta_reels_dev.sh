...
REPO_ROOT="/usr/local/development/instagram-reel-creation"
...
cd /usr/local/development/instagram-reel-creation/frontend

"$NODE_BIN/npm" run dev
pid_frontend=$!

trap 'kill "$pid_api" "$pid_worker" "$pid_frontend" 2>/dev/null || true; wait || true' SIGINT SIGTERM

# Exit if any process dies, and stop the rest.
wait -n "$pid_api" "$pid_worker" "$pid_frontend"
exit_code=$?
kill "$pid_api" "$pid_worker" "$pid_frontend" 2>/dev/null || true
wait || true
exit "$exit_code"
